    ...
    <div class="invoice-container">
      ...
      <!-- Company Details Section -->
        ...
        <table class="items-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 120px;">Customer<br>Name</th>
                    <th style="width: 60px;">Machine<br>No</th>
                    <th style="width: 80px;">Size</th>
                    <th style="width: 60px;">Micron</th>
                    <th style="width: 60px;">Rolls<br>Done</th>
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $index + 1 }}</td>
                    <td>{{ date('d/m/y H:i', strtotime($item->production_datetime)) }}</td>
                    <td>{{ $item->customer ? $item->customer->customer_name : 'N/A' }}</td>
                    <td>{{ $item->machine_number }}</td>
                    <td>{{ $item->size }}</td>
                    <td>{{ $item->micron }}</td>
                    <td>{{ $item->rolls_done }}</td>
                    <td class="amount-col">{{ number_format($item->kilograms_produced, 2) }}</td>
                </tr>
                @empty
                <tr>
                ...
                @endforelse
                <tr class="total-row">
                    <td colspan="7" style="text-align: right;"><strong>Total Production:</strong></td>
        ...
        {{-- <div class="amount-words-section">
            <strong>Total Production (in words):</strong><br>
